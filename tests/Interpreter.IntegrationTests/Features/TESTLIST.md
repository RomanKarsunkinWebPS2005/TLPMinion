...
- [x] Вложенные ветвления из файла (`if_branches.minion`)
...
- [x] Унарный `!`
- [x] `==`, `!=`, `<`, `<=`, `>`, `>=` для `Int`, `Float`, `String`
- [x] `&&`, `||`, `?:`
- [x] short-circuit: `false && ((1/0)==1)`, `true || ((1/0)==1)`
- [x] `1 == 1.0` — ошибка типов; `true && ((1/0)==1)` — деление на ноль

---

## Ветвления (`IfStatementsTest.cs`)

- [x] `if (true) { … }` выполняет тело
- [x] `if (false) { … }` пропускает тело
- [x] `if` / `else` по результату сравнения
- [x] цепочка `else if` / `else`
- [x] присваивание во внешнюю переменную из ветки `if`
- [x] объявление `var` внутри ветки
- [x] затенение в блоке внутри `if`
- [x] short-circuit в условии `if`: `false && (1/0)` не выполняется
- [x] код после `if` / `else`
- [x] сценарий из файла `if_branches.minion` (`else { if … }`)
- [x] сценарий из файла `if_branches_else_if.minion` (`else if`)
- [x] условие не `Bool` → ошибка семантики
- [x] `true && (1/0)` в условии `if` → деление на ноль
...
- [x] `<`, `>`, `<=`, `>=`, `==`, `!=` для литералов
- [x] регистр: `"A" < "a"`, `"A" > "a"`
- [x] префикс и длина: `"ab" < "b"`, `"ab" > "a"`
- [x] кириллица: `"а" < "я"`
- [x] символ вне BMP (emoji): `"a" < "🙂"`
