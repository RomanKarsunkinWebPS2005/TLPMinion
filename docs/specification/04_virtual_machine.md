...
#### Логические операции/сравнения/равенства

> Для операторов `&&` и `||` в codegen не использует `And` и `Or`: оба операнда должны вычисляться по правилам short-circuit, поэтому генерируются переходы `JumpIfTrue` / `JumpIfFalse`. Инструкции `And` и `Or` — примитивы VM используемые на начальном этапе

15. `And` выполняет бинарную операцию «логическое и»
   *   Снимает `EVAL[^2]` и `EVAL[^1]`; оба операнда должны быть типа `Bool`
   *   На стек помещает одно значение типа `Bool`

16. `Or` выполняет бинарную операцию «логическое или»
   *   Снимает `EVAL[^2]` и `EVAL[^1]`; оба операнда должны быть типа `Bool`
   *   На стек помещает одно значение типа `Bool`

17. `Equal` выполняет бинарную операцию "равно"
   *   Вычисляет `EVAL[^2]` `==` `EVAL[^1]`
   *   Снимает два значения со стека, результат помещает в стек

18. `NotEqual` выполняет бинарную операцию "не равно"
   *   Вычисляет `EVAL[^2]` `!=` `EVAL[^1]`
   *   Снимает два значения со стека вычислений, результат помещает в стек вычислений

19. `Less` выполняет бинарную операцию "меньше"
   *   Вычисляет `EVAL[^2]` `<` `EVAL[^1]`
   *   Снимает два значения со стека вычислений, результат помещает в стек вычислений

20. `LessOrEqual` выполняет бинарную операцию "меньше или равно"
   *   Вычисляет `EVAL[^2]` `<=` `EVAL[^1]`
   *   Снимает два значения со стека вычислений, результат помещает в стек вычислений
...
23. `JumpIfTrue [Target]` выполняет условный переход, если условие истинно
   *   Операнд хранит номер инструкции, на которую выполняется переход
   *   Переход выполняется, если `EVAL[^1]` — `true`
   *   Снимает одно значение типа `Bool` из стека

24. `JumpIfFalse [Target]` выполняет условный переход, если условие ложно
   *   Операнд хранит номер инструкции, на которую выполняется переход
   *   Переход выполняется, если `EVAL[^1]` — `false`
   *   Снимает одно значение типа `Bool` из стека

#### Вызов функций

25. `CallBuiltin [Code]` вызывает встроенную функцию
...
26. `StoreResult` запоминает результат выполнения программы
