...
15. `Not` выполняет унарную операцию логического отрицания
    * Снимает значение со стека, результат помещает в стек
...
25. `CallBuiltin [Code]` вызывает встроенную функцию
    *   Операнд — целый код BuiltinFunctionCode: Print, Abs, Min, Max, InputInt, InputFloat, InputBool, InputString
...
26. `Call [Index]` вызывает пользовательскую функцию
    *   Операнд хранит индекс инструкции, с которой начинается функция
    *   Сохраняет текущий указатель инструкций и текущую таблицу переменных в стек возвратов.
    *   Аргументы берутся со стека: `EVAL[^1]` — последний аргумент, `EVAL[^N]` — первый
    *   Возвращаемое значение (если есть) кладётся на стек (`EVAL[^1]`)

27. `Return` завершает текущую функцию
...
28. `StoreResult` запоминает результат выполнения программы
    *   Снимает `EVAL[^1]` и запоминает как результат выполнения программы

29. `Halt` останавливает работу программы
